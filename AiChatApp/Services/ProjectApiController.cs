...
public static class ProjectApiController
{
    public static void MapProjectEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapPost("/", async ([FromForm] string name, [FromForm] string rootPath, ProjectService projectService, ClaimsPrincipal user) =>
        {
            ...
            var html = $@"
                <div class='flex items-center group w-full mb-1'>
                    ...
                    <button onclick='openProjectEditModal({p.Id}, ""{p.Name}"", ""{p.RootPath}"")' title='Edit' class='btn btn-ghost btn-xs opacity-0 group-hover:opacity-60 px-1'>
        ...
        group.MapPut("/{id}", async (int id, [FromForm] string name, [FromForm] string rootPath, ProjectService projectService, ClaimsPrincipal user) =>
        {
            var userId = int.Parse(user.FindFirstValue(ClaimTypes.NameIdentifier)!);
            var project = await projectService.UpdateProjectAsync(id, name, rootPath, userId);
            if (project == null) return Results.NotFound();
            return Results.Ok(new { project.Id, project.Name, project.RootPath });
        }).DisableAntiforgery();
