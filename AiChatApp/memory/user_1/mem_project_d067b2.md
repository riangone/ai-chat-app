---
name: project,git,branch,NetYamlForge,photo-vault
description: 项目 NetYamlForge（包含子项目 photo-vault）在 Git 的 nyf 分支上进行开发。
type: user
userId: 1
tags: project,git,branch,NetYamlForge,photo-vault
relations: NetYamlForge,photo-vault,nyf
relevanceScore: 70
accessCount: 0
createdAt: 2026-06-21T23:28:09.0470746Z
lastAccessedAt: 2026-06-21T23:28:09.0470747Z
boundAgentRole: 
---

项目 NetYamlForge（包含子项目 photo-vault）在 Git 的 nyf 分支上进行开发。